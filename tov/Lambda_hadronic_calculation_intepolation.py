...
p1p2p3=np.array(cPickle.load(f_file)[0])
f_file.close()
...
p1_range=(p1p2p3[:,0,0,0].min(),p1p2p3[:,0,0,0].max())
...
def get_p1p2p3_i(p1,p2,p3):
    p1_i=(p1-p1_range[0])/(p1_range[1]-p1_range[0])*shape[0]
    p2_max=causality_p2_int(p1)
    p2_i=(p2-p2_range[0])/(p2_max-p2_range[0])*shape[1]
    log_p3_min=np.log(np.max([1.2*p2,250]))
    log_p3_max=np.log(p3_max(p1,p2))
    p3_i=(np.log(p3)-log_p3_min)/(log_p3_max-log_p3_min)*shape[2]
...
log_Lambda_mass_grid=cPickle.load(f_file)
f_file.close()
